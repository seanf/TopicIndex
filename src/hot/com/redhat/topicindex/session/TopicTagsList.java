...
public class TopicTagsList extends ExtendedTopicList {
	 ...
	 */
	@Override
	protected void construct(final int limit, final String constructedEJBQL,
			final TopicFilter topic) {
	...
	public void applyBulkTags() {
		final TopicTagsList cloneList = new TopicTagsList(-1,
				this.constructedEJBQL, this.topic);
		...
		for (final Boolean remove : new Boolean[] { true, false }) {
			...
			for (final Topic topic : bulkTagList) {
				for (final UIProjectCategoriesData project : selectedTags
					...
					for (final UICategoryData entry : project.getCategories()) {
						...
						for (final UITagData tagDetails : entry.getTags()) {
							...
							// selected
							if ((tagDetails.isNotSelected() && remove)
									|| (tagDetails.isSelected() && !remove)) {
	...
	protected void manageTag(final Topic topic, final UITagData tagDetails,
			final boolean remove) {
		final EntityManager entityManager = (EntityManager) Component
				.getInstance("entityManager");
		final Tag tag = entityManager.getReference(Tag.class,
				tagDetails.getId());
		final Topic persistTopic = entityManager.find(Topic.class,
				topic.getTopicId());

		// remove tags
		if (remove && tagDetails.isNotSelected()) {
			with(persistTopic.getTopicToTags()).remove(
					having(on(TopicToTag.class).getTag(), equalTo(tag)));
		}
		// add tags
		else if (!remove && !tagDetails.isNotSelected()) {
			if (filter(having(on(TopicToTag.class).getTag(), equalTo(tag)),
					persistTopic.getTopicToTags()).size() == 0)
	...
	public void addTagById(final Integer topicID, final Integer tagID) {
		try {
			...
			if (filter(having(on(TopicToTag.class).getTag(), equalTo(tag)),
					topic.getTopicToTags()).size() != 0) {
				with(topic.getTopicToTags()).remove(
						having(on(TopicToTag.class).getTag(), equalTo(tag)));
			} else {
				...
				for (final Tag excludeTag : tag.getExcludedTags()) {
					...
					with(topic.getTopicToTags()).remove(
							having(on(TopicToTag.class).getTag(),
									equalTo(excludeTag)));
				}

				for (final TagToCategory category : tag.getTagToCategories()) {
					if (category.getCategory().isMutuallyExclusive()) {
						for (final TagToCategory categoryTag : category
								.getCategory().getTagToCategories()) {
							if (categoryTag.getTag().equals(tag))
								continue;

							with(topic.getTopicToTags()).remove(
									having(on(TopicToTag.class).getTag(),
											equalTo(categoryTag.getTag())));
						}
					}
				}
			}
	...
	protected void removeRelationship(final boolean to) {
		if (actionTopicID != null && otherTopicId != null) {
			try {
				final EntityManager entityManager = (EntityManager) Component
						.getInstance("entityManager");
				final Topic topic = entityManager.find(Topic.class,
						actionTopicID);
				final Topic otherTopic = entityManager.find(Topic.class,
						otherTopicId);
		...
		try {
			if (actionTopicID != null && otherTopicId != null) {
				EntityManager entityManager = (EntityManager) Component
						.getInstance("entityManager");
				final Topic actionTopic = entityManager.find(Topic.class,
						actionTopicID);
				final Topic otherTopic = entityManager.find(Topic.class,
						otherTopicId);
		...
		try {
			if (actionTopicID != null) {
				...
				for (final TopicToTag topicToTag : topic.getTopicToTags()) {
					...
					topicTags += "[" + topicTag.getTagId() + "] "
							+ topicTag.getTagName() + "\n";
				}

				for (final TopicToTopic topicToTopic : topic
						.getParentTopicToTopics()) {
					final Topic relatedTopic = topicToTopic.getRelatedTopic();
					topicTags += "\n\n[Tags From TopicID "
							+ relatedTopic.getTopicId() + "]\n";
					for (final TopicToTag relatedTopicToTag : relatedTopic
							.getTopicToTags()) {
						final Tag relatedTag = relatedTopicToTag.getTag();
						topicTags += "[" + relatedTag.getTagId() + "] "
								+ relatedTag.getTagName() + "\n";
					}
				}
				...
				for (final TopicToTopic topicToTopic : topic
						.getParentTopicToTopics()) {
					final Topic relatedTopic = topicToTopic.getRelatedTopic();
					topicUrls += "\n\n[TopicURLs From TopicID "
							+ relatedTopic.getTopicId() + "]\n";
					for (final TopicToTopicSourceUrl url : relatedTopic
							.getTopicToTopicSourceUrls())
						topicUrls += url.getTopicSourceUrl() + "\n";
				}

				// the topic text will contain the details of the other topics
				topic.setTopicText(topicTags + topicText + topicTitle
						+ topicAddedBy + topicUrls);
				...
				for (final TopicToTopic topicToTopic : topic
					...
					for (final TopicToTag relatedTopicToTag : relatedTopic
						...
						if (filter(
								having(on(TopicToTag.class).getTag(),
										equalTo(relatedTopicToTag.getTag())),
								topic.getTopicToTags()).size() == 0)
							topic.getTopicToTags().add(
									new TopicToTag(topic, relatedTopicToTag
											.getTag()));
					}
				}
	...
	public String getRelatedTopicsUrl() {
		try {
			if (actionTopicID != null) {
				return "/CustomSearchTopicList.seam?topicRelatedTo="
						+ actionTopicID;
			}
		} catch (final Exception ex) {
			ExceptionUtilities.handleException(ex);
